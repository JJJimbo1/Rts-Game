...
pub struct Factory {
    ...
    #[superstruct(only(Bundle))]                pub factory: Factory,
...
impl From<(FactoryDisk, &FactoryPrefab)> for FactoryBundle {
    fn from((save, prefab): (FactoryDisk, &FactoryPrefab)) -> Self {
        Self {
            factory: Factory,
            // object_type: FactoryMarker.into(),
            object_type: Factory.into(),
...
impl<'a> From<FactoryDiskQuery<'a>> for FactoryDisk {
    fn from(object: FactoryDiskQuery) -> Self {
        Self {
