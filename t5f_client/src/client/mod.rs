...
use t5f_common::{ClientRequest, ClientRequests, LoadObjects, ServerCommand, ServerCommands, SpawnObjects, CLIENT_ADDRESS, SERVER_ADDRESS};
    ...
    fn read_messages(
        ...
        while let Some(Some(bytes)) = (loop_count < 20).then(|| client.receive_message(DefaultChannel::ReliableOrdered)) {
            ...
            for messege in payload.commands {
                ...
                client_events.send(messege.clone());
                match messege {
                    ServerCommand::Empty => {},
                    ServerCommand::SpawnObject(event) => {
                        load_events.send(event.into());
                    }
                }
            }
            loop_count += 1;
