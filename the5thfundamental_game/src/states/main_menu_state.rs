        ...
        for event in campaign_button_event_reader.iter() {
            match *event {
                CampaignButtonsEvent::Continue => {
                    commands.insert_resource(ChosenSaveFile("saves/developer.t5flvl".to_string()));
...
impl Plugin for MainMenuPlugin {
    fn build(&self, app: &mut App) {
        app.add_system_set(SystemSet::on_enter(GameState::MainMenu)
            .with_system(Self::spawn_camera)
            .with_system(create_main_menu)
        )
            .add_system_set(SystemSet::on_update(GameState::MainMenu)
            .with_system(button_updater_system)
            .with_system(main_menu_ui_button_event_writer_system.after(button_updater_system))
            .with_system(Self::handle_buttons.after(main_menu_ui_button_event_writer_system))
        )
...
//         .with_system(main_menu_ui_button_event_writer_system.label(MainMenuSystems::MainMenuEventUpdater).after(MainMenuSystems::ButtonUpdater))
