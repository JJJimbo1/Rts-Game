...
impl Plugin for MatchLoadingStatePlugin {
    fn build(&self, app: &mut App) {
        app.add_system_set(SystemSet::on_enter(GameState::MatchLoadingState)
            .with_system(Self::load_level)
        )
            .add_system_set(SystemSet::on_update(GameState::MatchLoadingState)
            .with_system(Self::finish_loading)
        );
    }
}
