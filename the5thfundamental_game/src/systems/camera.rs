...
use bevy::{input::{mouse::{MouseWheel, MouseButtonInput}, ButtonState}, prelude::*, render::camera::Camera, math::Vec3Swizzles, ecs::schedule::StateData};
use bevy_ninepatch::*;
...
pub struct CameraPlugin<S: StateData> {
    state: S
}

impl<S: StateData> CameraPlugin<S> {
...
impl<S: StateData> Plugin for CameraPlugin<S> {
    fn build(&self, app: &mut App) {
        ...
        .add_system_set(SystemSet::on_enter(self.state.clone()).label(SystemSets::Camera)
            .with_system(Self::create_camera)
            .with_system(Self::create_selector))

        .add_system_set(SystemSet::on_update(self.state.clone()).label(SystemSets::Camera)
            .with_system(Self::camera_control_system)
            .with_system(Self::ui_hit_detection_system.after(Self::camera_control_system))
            .with_system(Self::camera_raycast_system.after(Self::ui_hit_detection_system))
            .with_system(Self::building_placement_system.after(Self::camera_raycast_system))
            .with_system(Self::camera_raycast_response_system.after(Self::building_placement_system))
            .with_system(Self::show_selection_box.after(Self::camera_raycast_response_system))
            .with_system(Self::camera_select.after(Self::show_selection_box))
            .with_system(Self::camera_context_focus_system.after(Self::camera_raycast_response_system))
            .with_system(Self::selection_highlighter.after(Self::camera_raycast_response_system))
            .with_system(Self::command_system.after(Self::camera_raycast_response_system))
        );
    }
}
