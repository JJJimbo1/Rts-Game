...
pub struct ContextMenuPlugin<S: StateData> {
    state: S
}

impl<S: StateData> ContextMenuPlugin<S> {
...
impl<S: StateData> Plugin for ContextMenuPlugin<S> {
    fn build(&self, app: &mut App) {
        app
        .add_system_set(SystemSet::on_enter(self.state.clone())
            .with_system(Self::create_context_menu)
        )
        .add_system_set(SystemSet::on_update(self.state.clone())
            .with_system(Self::context_menu_update.after(QueueSystem))
            .with_system(Self::context_menu_event_writer.after(Self::context_menu_update))
            .with_system(Self::context_menu_event_reader.after(Self::context_menu_event_writer))
        );
    }
}
